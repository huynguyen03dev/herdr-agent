...
2. **Bootstrap your wake path once, on entry, before dispatching anyone.** The
   ...
   unnamed — so name it yourself and confirm the broker is live:
   ```bash
   herdr agent rename "$HERDR_PANE_ID" root   # so done/blocked wakes can reach you
   herdr plugin list | grep -q attention-broker || echo "NO BROKER: wakes will not fire"
   ```
   If the broker is absent, you will receive no completion wakes — say so, and
   fall back to the bounded backstop in §7 (never a poll loop). Do not rely on
   memory for this: a cold root with no memory must still establish its own wake
   path from this instruction alone.
...
5. Only **one** personnel protocol runs this room — Herdr. Do not also rely on a
   sub-agent framework. Co-workers must not spawn their own co-workers.
6. You keep the final synthesis and the answer to the user. That is always yours.
...
sent, stop actively watching. Completion is *pushed* to you: in a room that runs
the attention-broker (this one does), a co-worker reaching `done` or `blocked`
wakes you with a `HERDR_ATTENTION_EVENT <seat>:done` message naming the seat.
That wake is your signal to read — until it arrives, drive other streams or let
the turn rest. Do **not** sit in a blocking `herdr agent wait` or a poll loop to
watch one co-worker finish: it freezes your context, burns tokens on an
unchanged state, and re-creates the polling waste the broker exists to remove
(the *frozen-wait* anti-pattern).
...
decays back to `idle`. Treat the *wake*, not `idle`, as completion: a freshly
spawned co-worker also sits at `idle`, so idle alone is ambiguous.

The broker has no deadline timer, so a co-worker that hard-crashes or freezes
without ever emitting `done`/`blocked` never wakes you. For that case only, use a
**single bounded** backstop — one `herdr agent get "$PANE"`, or one
`herdr agent wait "$PANE" --until done --until idle --timeout <ms>` (this is the
real flag; there is no `herdr wait agent-status`) — never a repeated poll. If the
backstop settles on `idle` after a dispatched task, that is itself a completion,
not a stall.
...
19. Always prefer transparency: clear is better than clever.
